...
**Wasona needs translators!** Wasona is currently available in {(() => {
  let enLangNames = new Intl.DisplayNames("en", {type: "language"});
  return new Intl.ListFormat("en").formatToParts(langs).map(({ type, value }) =>
    type === "literal" ? value : <a href={getRelativeLocaleUrl(value)}>{enLangNames.of(value)}</a>);
})()}. If you want to add a new translation, tell me on Discord (@kala_asi) or via email (kala.asi.la@gmail.com). I'll help out in any way I can!
