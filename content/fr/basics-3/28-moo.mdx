---
slug: "fr/28"
title: "leçon 28 : meuh !"
description: "Apprenez taso, n, mu, pu, ku, kijetesantakalu!"
practice:
  - l1: "Je ne vois qu'un canard turquoise."
    l2: "mi lukin e waso laso taso."
    junkChips: ["pi"]
    alsoAccept: ["mi lukin e waso taso laso."]
  - l1: "Le raton-laveur t'aime."
    l2: "kijetesantakalu li olin e sina."
    junkChips: ["pona"]
  - l1: "Si tu veux, lis 'Toki Pona : la langue du bien' de Sonja Lang."
    l2: "sina wile la o pu."
    junkChips: ["li"]
  - l1: "Mais si tu ne veux pas, pas de problème."
    l2: "taso sina wile ala la, ale li pona."
    junkChips: ["ona"]
---

import Sentence from "@/components/Sentence.astro";
import List from "@/components/List.astro";
import Word from "@/components/Word.astro";
import Practice from "@/components/Practice.astro";

Il ne nous reste qu'un mot à voir, avant de passer aux choses non-sérieuses.

<List>
  <Word sl="taso" m="mais, juste, ne ... que" />
</List>

**󱥨 taso** peut être un adjectif signifiant "juste, seulement" (quelquefois traduit par "ne ... que"), mais il peut également signifier "mais", lorsqu'il est placé en début de phrase. 

<List>
  <Sentence sl="mi awen wan taso." m="Je reste seul." />
  <Sentence sl="taso mi awen pilin pona." m="Mais je vais toujours bien." />
</List>

## Passons aux choses non-sérieuses !

**󱦆 n** est équivalent à **󱤀 a**, mais en parlant du nez :

<List>
  <Word sl="n" m="hum, euh" />
  <Sentence sl="n... mi sona ala." m="hum... Je ne sais pas." />
</List>

**󱤹 mu** est le bruit que fait un animal ! Mais lequel ? Et bien, tous les animaux :

<List>
  <Word sl="mu" m="meuh, miaou, ouaf, cui-cui..." />
  <Sentence sl="soweli mute! mu mu mu mu mu mu." m="Six vaches ! <a target='_blank' href='https://www.youtube.com/watch?v=xxClPfZDn-E'>Meuh meuh meuh meuh meuh meuh</a>." />
</List>

Si les gens utilisent **󱤹 mu** de manière ludique, **󱤹 mu** a aussi des emplois plus sérieux. **󱤹 mu** sert à imiter un bruit (là où **󱤕 kalama** sert à décrire un bruit) :


<List>
  <Sentence sl="mi kute e kalama suli." m="J'ai entendu un grand bruit." />
  <Sentence sl="mi kute e mu suli." m="J'ai entendu un grand boum." />
</List>

## Encore plus de mots

Nous avons l'ensemble des mots que vous *devez* connaître pour parler toki pona. Mais d'autres sont aussi utilisés. Pourquoi ? Parce que tout n'est pas conçu pour être efficace, il faut aussi savoir s'amuser ! Et il n'y a pas de mot qui montre celà plus que : 

<List>
  <Word sl="kijetesantakalu" m="raton-laveur ; animal ressemblant à un raton-laveur (bassari, panda roux, kinkajou)" />
</List>

En plus d'être très long, ce mot est très spécifique (bien plus que les autres mots de toki pona). Bien qu'il vienne d'un poisson d'avril datant de 2009, le mot est resté et les tokiponistes l'utilisent encore, des fois comme mascotte.

Trois mots nécessitent une leçon d'histoire du toki pona :

<List>
  <Word sl="pu" m="(intéragir avec) <i>Toki Pona: La langue du bien (2014)</i>, de Sonja Lang" />
  <Word sl="ku" m="(intéragir avec) <i>Toki Pona Dictionary (2021)</i>, de Sonja Lang" />
</List>

Ces mots renvoient à des livres spécifiquement écrits ou produits par la créatrice du toki pona [Sonja Lang](https://tokipona.org) (lien en anglais). Pourquoi ont-ils des mots spécialisés ? Probablement parce que c'est drôle. Et puis, les tokiponistes adorent parler de ce qui est dans les livres et de ce qui n'y est pas, donc ces mots sont utiles.

Si vous rejoignez une communauté de tokiponistes, vous verrez certainement plus de mots. Dans ce cas, considérez-les comme des *artefacts culturels*. Ils n'existent pas pour être réellement utilisés, mais plutôt pour expérimenter, s'amuser, être créatif, faire des blagues.

<List>
  <Sentence sl="kijetesantakalu tonsi li lanpan ala lanpan e soko?" m="Est-ce que le raton-laveur non-binaire vole des champignons ?" />
</List>

Vous ne connaissez probablement pas le mot **󱦅 lanpan**, mais vous pouvez regarder la traduction et réfléchir à un mot que vous connaissez qui pourrait le remplacer (**󱤖󱤓 kama jo**) ! C'est généralement la même chose pour les autres mots sur lesquels vous pourriez tomber : il y a toujours un moyen de le dire en toki pona de base.

<Practice exercises={frontmatter.practice} />

## Voir aussi
* [FAQ : Quels sont les autres mots rares que je pourrais rencontrer ?](/fr/uncommon-words/)
* [Que devient Sonja Lang maintenant ?](https://tokipona.org/madame_lang)
* [Vidéo: tous les animaux font **󱤹 mu**](https://www.youtube.com/watch?v=zh-kLJI43xw)
